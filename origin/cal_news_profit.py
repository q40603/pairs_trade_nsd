...
year.sort()
header = ["used_news", "time", "id", "profit", "capital", "trade", "position", "s1_price", "s2_price", "re"]
dt = {"used_news" : bool, "time": str, "profit": float, "trade" : int, "position" : int, 'captial': float, "re" : bool} 
df = pd.read_csv(sys.argv[1], names=header, sep=" ", dtype=dt, infer_datetime_format=True, error_bad_lines=False)
# df["time"] = pd.to_datetime(df["time"])
# 
print(df[df['used_news']==True])
#print(data)
for y in range(2015,2020):
    tmp = df[(df['time'].str.startswith(str(y))) & (df['used_news']==True)]
    print("---------- {} --------------------------".format(y))
