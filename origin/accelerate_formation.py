...
#        self.z_c_r = np.zeros(len(self.spread.T))
        ...
        con = pd.concat([con,self.s_n_r,self.ave,self.std,self.ske],axis=1)
    ...
    def formation_period(self):
        ...
        if self.find_pairs():
            ...
            result = pd.DataFrame(columns=["stock1","stock2","model_type","w1","w2","pval","snr","mu","stdev","skewness","Emu","Estd","action"])
