...
def pairs(pos, formate_time, table, min_data, tick_data, maxi, tax_cost, cost_gate, capital):
    ...
    return local_rt, trading_profit
...
################## query table ##################
    ...
    query = "select * from pairs where f_date = '{}';".format(choose_date)
    fin_cursor.execute(query)
    result = fin_cursor.fetchall()
    fin_db.commit()
    table = pd.DataFrame(list(result))    
    ...
    for i in range(len(table)):
        result = trade_pair(choose_date, table.stock1[i], table.stock2[i])
        print(result)
        update = "UPDATE pairs set rt = {}, profit = {} where id={};".format(result[0], result[1] , table.id[i])
        fin_cursor.execute(update)
        fin_db.commit()